...
class FileStore:
    ...
    def __init__(
        ...
        self.primary_id = pag.locateCenterOnScreen(
            primary_id_img, confidence=0.9, region=self.region
        )
    ...
    def _start(self, import_button_img: Path, import_button_check_img: Path) -> None:
    ...
    def keyword_boxes(self, info: list) -> None:
        ...
        try:
            center = pag.locateCenterOnScreen(
                self.event_date_img, confidence=0.9, region=self.region
            )
